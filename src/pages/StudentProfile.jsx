    ...
    useEffect(() => {
        const fetchSkillGap = async () => {
            try {
                ...
                if (!response.ok) throw new Error('Tiada data pelajar dijumpai untuk akaun ini.');
                ...
                {loading ? (
                    ...
                    <>
                            ...
                            <div className="lg:col-span-2 bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-100 flex flex-col md:flex-row gap-6 items-start md:items-center">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-1">
                                        <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Pelajar {studentId}</h1>
                                        <span className={`border px-3 py-1 rounded-full text-xs font-bold ${skillGap?.student.dropoutRisk === 'Tinggi' ? 'bg-red-100 text-red-700 border-red-200' : skillGap?.student.dropoutRisk === 'Sederhana' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-green-100 text-green-700 border-green-200'}`}>{skillGap?.student.dropoutRisk || 'Berisiko Tinggi'}</span>
                                    </div>
                                    <p className="text-slate-500 font-mono text-sm mb-3">No. Matrik: {studentId}</p>
                                    
                                    <div className="grid grid-cols-2 gap-y-2 gap-x-6 text-sm">
                                        <p className="text-slate-600"><span className="font-semibold">Kursus:</span> Diploma Computer Technology (Cloud Computing)</p>
                                        <p className="text-slate-600"><span className="font-semibold">Semester:</span> Semester 2</p>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                                <h3 className="font-bold text-slate-800 mb-4">Radar Kemahiran Individu</h3>
                                <div className="h-64 w-full flex justify-center">{skillData && <Radar data={skillData} options={{ maintainAspectRatio: false }} />}</div>
                            </div>
                        </div>
